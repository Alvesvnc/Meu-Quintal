...
checar(
  'Marina ve so o quintal dela',
  meMarina.spaces.length === 1 && meMarina.spaces[0].slug === 'sao-sebastiao',
  meMarina.spaces.map((s) => s.slug).join(','),
);
checar(
  'Roberto ve so o quintal dele',
  meRoberto.spaces.length === 1 && meRoberto.spaces[0].slug === 'ubatuba-centro',
  meRoberto.spaces.map((s) => s.slug).join(','),
);
...
checar(
  'Roberto pedindo o quintal da Marina -> 404',
  cruzado.status === 404,
  `HTTP ${cruzado.status}`,
);
...
checar(
  'Roberto pedindo o proprio quintal -> 200',
  proprio.status === 200,
  `HTTP ${proprio.status}`,
);
...
checar(
  'Marina ve 6 cozinhas, Roberto 1',
  cozMarina.length === 6 && cozRoberto.length === 1,
  `${cozMarina.length} vs ${cozRoberto.length}`,
);
...
checar(
  'Roberto alterando a comissao da cozinha da Marina -> 404',
  ataque.status === 404,
  `HTTP ${ataque.status}`,
);
...
checar(
  'comissao da Marina intacta (15%, nao 99%)',
  louDepois.acordo.commissionPctEfetivo === 15,
  `${louDepois.acordo.commissionPctEfetivo}%`,
);
...
checar(
  'staff nas mesas -> 200 (e o trabalho dele)',
  staffMesas.status === 200,
  `HTTP ${staffMesas.status}`,
);
...
checar(
  'JWT de cozinha em rota de dono -> 403',
  cozNoDono.status === 403,
  `HTTP ${cozNoDono.status}`,
);
const donoNaCozinha = await get('/api/r/fila', marina);
checar(
  'JWT de dono em rota de cozinha -> 403',
  donoNaCozinha.status === 403,
  `HTTP ${donoNaCozinha.status}`,
);
...
checar(
  'Marina usa 15%',
  finM.linhas.find((l) => l.kitchenSlug === 'lou-burger').commissionPct === 15,
);
checar(
  'Roberto usa 20%',
  finR.linhas.find((l) => l.kitchenSlug === 'lou-burger').commissionPct === 20,
);
checar(
  'faturamentos nao se misturam',
  finM.totais.grossCents !== finR.totais.grossCents || finR.totais.grossCents === 0,
);
...
checar(
  'mesa de Ubatuba ve o quintal de Ubatuba',
  quintalUba.space?.slug === 'ubatuba-centro',
  quintalUba.space?.slug,
);
checar(
  'e so a cozinha de la',
  quintalUba.kitchens?.length === 1,
  `${quintalUba.kitchens?.length} cozinha(s)`,
);
