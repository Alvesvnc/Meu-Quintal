...
import { useAcesso, useDefinirSenha } from '../api/hooks';
...
function Formulario({ token, dados }: { token: string; dados: PrimeiroAcessoResponse }) {
  ...
  const enviar = (e: React.FormEvent) => {
    e.preventDefault();
    if (!pode) return;
    // Vai pra fila: e a tela onde a cozinha trabalha.
    definir.mutate(
      { token, password: senha },
      { onSuccess: () => navigate('/fila', { replace: true }) },
    );
  };
  ...
  return (
    <Moldura>
      ...
      <p className="mt-3 font-sans text-body text-inkMuted text-pretty">
        {recuperando ? (
          <>
            Você está trocando a senha de acesso à <span className="text-ink">{dados.accountName}</span>.
          </>
        ) : (
          <>
            Sua cozinha <span className="text-ink">{dados.accountName}</span> está pronta. Crie
            sua senha e você já entra.
          </>
        )}
      </p>
      ...
      <p className="mt-6 font-sans text-body-sm text-inkDim text-pretty">
        {recuperando
          ? 'Salvar aqui desconecta os outros aparelhos onde essa cozinha estava aberta.'
